...
const Pow5 = Utils_1.range(10).map(x => Math.floor(Math.pow(5, x)));
function huByTable(cards) {
    //基于查表的胡牌算法,cards可以无序
    let pattern = Utils_1.li(10, 0); //10个part，每个part的编码
    let partCardCount = Utils_1.li(10, 0); //10个part，每个part的牌数
    for (let i of cards) {
        const card = Card_1.CardMap[i];
