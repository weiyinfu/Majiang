...
function buildNumberCards() {
    ...
    function solvable(a: number[]) {
        ...
        function go(a: number[], ind: number, canJiang: boolean): boolean {
            if (ind === a.length) {
                //遍历结束
                return true;
            }
            if (a[ind] === 0) {
                return go(a, ind + 1, canJiang);
            }
            if (canJiang && a[ind] >= 2) {
                a[ind] -= 2;
                const res = go(a, ind, false);
                a[ind] += 2;
                if (res) return true;
            }
            if (a[ind] >= 3) {
...
const Pow5 = range(10).map(x => Math.floor(Math.pow(5, x)))

export function huByTable(cards: string[]): boolean {
    //基于查表的胡牌算法,cards可以无序
