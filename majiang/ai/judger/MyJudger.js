...
const TableHu_1 = require("../../hu/TableHu");
const MAX_STEPS = 100000; //到达胜利的步数
function buildReverseIndexOfPatterns(patterns) {
...
exports.buildReverseIndexOfPatterns = buildReverseIndexOfPatterns;
function buildReverseIndex() {
    //10个分区，每个分区都有可能有解的局面
    //为每个分区建立 牌的张数=>patterns倒排索引
    //字牌pattern列表
    const zi = buildReverseIndexOfPatterns([[0], [2], [3]]);
    const shuzi = buildReverseIndexOfPatterns(TableHu_1.shuziPai);
...
function vectorize(a) {
    ...
    for (let i = 0; i < a.length; i++) {
        const x = a[i];
        const card = Card_1.C.byName(x);
        if (!card)
            throw new Error(`no card ${x}`);
...
function splitVector(cardVector) {
    //把一维向量转成10个part的一维向量
    if (cardVector.length !== 34)
        throw new Error('error vector length');
    const parts = [];
    for (let i = 0; i < 7; i++) {
        //字牌
...
exports.splitVector = splitVector;
...
function stringifyPart(part, a) {
    //把一个part的向量转成字符串数组
    const cards = [];
    for (let i = 0; i < a.length; i++) {
        for (let j = 0; j < a[i]; j++)
            cards.push(Card_1.C.byPartOrdinal(part, i).name);
    }
    return cards;
}
function diff(target, now) {
    ...
    if (target.length !== now.length)
        throw new Error(`error ${target.join(',')} | ${now.join(',')}`);
...
function diff2(target, now) {
    //直接对2层分区进行diff
    if (target.length !== now.length || target.length == 10)
        throw new Error('error');
    let ind = 0;
    const c = new Array(34);
    for (let i = 0; i < 10; i++) {
...
class PartSumJudger {
    ...
    judge(state, hand) {
         ...
         * */
        let s = 0;
        for (let partId = 0; partId < exports.BIG.length; partId++) {
            ...
            if (partHandCount === 0)
                continue;
            let minL1 = MAX_STEPS;
            const countPatterns = exports.BIG[partId].entries();
            while (1) {
                const res = countPatterns.next();
                if (res.done)
                    break;
...
exports.PartSumJudger = PartSumJudger;
class MyJudger {
    /**
     ...
     */
    visitNeibors(hand, maxCardCount, state, patternCallback) {
