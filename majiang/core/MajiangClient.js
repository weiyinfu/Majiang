...
function howToEat(hand, card) {
    ...
    const c = Card_1.C.byName(card);
    //把手牌映射为稀疏ID列表
    const handCards = hand.map(name => Card_1.C.byName(name).sparseIndex);
    function have(beg, end) {
        ...
        for (let sparseId = beg; sparseId < end; sparseId++) {
            ...
            try {
                ...
                if (ind === -1)
                    return null;
                serie.push(Card_1.C.byName(hand[ind]).name);
            }
            catch (e) {
                return null;
            }
...
class MajiangClient {
    ...
    onOver(req) {
        ...
        Card_1.sortCards(this.hand[this.me]);
        Card_1.sortCards(this.anGang[this.me]);
        for (let [serverCards, clientCards] of [[req.hand, this.hand], [req.anGang, this.anGang]]) {
            ...
            for (let i = 0; i < serverCards.length; i++) {
                Card_1.sortCards(serverCards[i]);
